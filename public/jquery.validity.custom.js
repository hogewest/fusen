...
// Install the customLabel output.
(function($) {
    var 
        // Erroneous refers to an input with an invalid value,
        // not the error message itself.
    ...
    function insertTarget($obj) {
      var id = getIdentifier($obj);

      if($.validity.settings.outputErrorTarget[id]) {
        return $("#" + $.validity.settings.outputErrorTarget[id]);
      }
      return $obj;
    }

    $.validity.outputs.customLabel = {
        start:function() {
            // Remove all the existing error labels.
            $("label." + $.validity.settings.cssClass).remove();
            $(errors).removeClass(erroneous);
